...
const UPLOAD_HOME_PICTURE = `
  mutation panelUpdateHomeImage($file: Upload! $initialImage: String!) {
    panelUpdateHomeImage(file: $file, initialImage: $initialImage) {
      id
    }
  }
`
...
const Starter = () => {
  ...
  useEffect(() => {
    if (data && data.panelGetSettings.homeImage) {
      setPreview(data?.panelGetSettings.homeImage)
    }
  }, [data])
  ...
  const onSaveImage = async () => {
    setLoading(true)
    //TODO: personalizar
    const result = await upload({ file: selectedFile, initialImage: '2' })
    if (result && result.panelUpdateHomeImage.id) {
      account.setAccount({ ...account, homeImage: preview })
  ...
  const removePicture = async () => {
    ...
    if (result && result.panelRemoveHomeImage.id) {
      account.setAccount({ ...account, homeImage: null })
  ...
  return (
    <>
      <Head title='Alterar Imagem Inicial do Aluno.Tv' />
      <div className='flex flex-col md:flex-row justify-between mt-4'>
        <Title text='Alterar Imagem Inicial do Aluno.Tv' />
      </div>
      <div className='mb-8 mt-6'>
        <div className='sm:px-4 py-3 bg-gray-100 rounded-lg shadow-md dark:bg-gray-800'>
          <div className='flex flex-col items-center justify-center max-w-lg mx-auto'>
            <div
              className='w-96 h-96 bg-gray-300 bg-center bg-cover rounded-lg shadow-md p-3'
