...
const EDIT_PASSWD_ADMIN = `
  mutation panelUpdateAdmin($id: Int!, $password: String) {
...
const Schema = Yup.object().shape({
  ...
  confirm: Yup.string().test(
    '',
    'A senha e confirmação da senha devem ser iguais.',
    (value, ctx) => {
...
const Create = () => {
  ...
  const [, edit] = useMutation(EDIT_PASSWD_ADMIN)
  const { data } = useQuery(GET_ADMIN(router.query?.id?.toString()))
  ...
  return (
    <>
      ...
      <div className='mt-4'>
        <Title
          text='Alterar Senha'
          subText={`Usuário: ${data?.panelGetAdminById?.name}`}
        />
      </div>
